...
async function sendMessage() {
    ...
    try {
        ...
        catch (e) { console.error("message_api.php returned:", text); alert("Server error. Check message_api.php."); return; }

        if (!data.ok) { alert(data.error || "Message could not be sent."); return; }
        input.value = "";
        await loadMessages();
    } catch (e) {
        console.error(e);
...
async function encryptAndSend() {
    ...
    try {
        ...
        if (!data.ok) { alert(data.error || "Encrypted message could not be sent."); return; }
        input.value = "";
...
async function loadMessages() {
    ...
    try {
        ...
        let data;
        try { data = JSON.parse(text); }
        catch (e) { console.error("message_api.php returned:", text); return; }

        if (!data.ok) { console.error(data.error || "Could not load messages."); return; }
        ...
        data.messages.forEach(function(message) {
            ...
            if (raw.indexOf("[[PRIVATE:") === 0) {
                ...
                if (end !== -1) {
                    const key = parseInt(raw.substring(10, end), 10);
                    const encrypted = raw.substring(end + 2);
                    div.dataset.encrypted = encrypted;
                    div.dataset.key = key;

                    if (decryptedCache.has(encrypted)) {
                        div.dataset.decrypted = "true";
                        div.innerHTML = "<b>🔓 Decrypted message</b><br><br>" + escapeHtml(decryptedCache.get(encrypted));
                    } else {
                        div.innerHTML = "<b>🔐 Encrypted message</b><br><br>" + escapeHtml(encrypted) + "<div class='message-key'>Key required to decrypt</div>";
                    }
                } else {
                    div.textContent = raw;
                }
...
function decryptSelected() {
    ...
    const actualKey = Number(div.dataset.key);
    const enteredKey = prompt("Enter the encryption key:");

    if (enteredKey === null) return;

    const key = Number(enteredKey.trim());
    if (!Number.isInteger(key) || key < 1 || key > 94) {
        alert("Please enter a valid key from 1 to 94.");
        return;
    }
    ...
    div.dataset.decrypted = "true";
    div.innerHTML = "<b>🔓 Decrypted message</b><br><br>" + escapeHtml(decrypted);
}
